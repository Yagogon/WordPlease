...
class UserSerializer(serializers.Serializer):
    ...
    def create(self, validated_data):
        """
        Crea una instancia de User a patir de los datos de
    ...
    def update(self, instance, validated_data):
        """
        Actualiza una instancia de User a patir de los datos
    ...
    def validate_username(self, data):
        ...
        :param data: username
        :return:data si es valido
        """
        users = User.objects.filter(username=data)

        # Si estoy creando (no hay instancia) comprobar si hay usuario con ese usernma
        if not self.instance and len(users) !=0:
            raise serializers.ValidationError('Ya existe un usuario con ese username')

        # Si estoy actualizando, el nuevo username es diferente al de la instancia (esta cambiando el username)
        # y existian usuario ya registrados con el nuevo username
